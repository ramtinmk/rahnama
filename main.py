...
def home():
    ...
    return render_template("/home.html", is_logged=is_logged, username=username,unseen_number=unseen_number)
...
def ask_question():
    ...
    if islogged:
        unseen_number = query_db("select COUNT(*) as count from notifications  where seen=0 and to_username= ? ;",[session["username"]],one=True)["count"]
    return render_template("/ask_question.html",unseen_number=unseen_number,is_logged=check_is_logged())
...
def myquestions():
    ...
    for post in my_posts:
        post["time_ago"] = time_ago(post["created_at"])
    unseen_number = query_db("select COUNT(*) as count from notifications  where seen=0 and to_username= ? ;",[username],one=True)["count"]
    return render_template("/myquestions.html", posts=my_posts,is_logged=is_logged,unseen_number=unseen_number)
...
def yourprofile():
    ...
    unseen_number = query_db("select COUNT(*) as count from notifications  where seen=0 and to_username= ? ;",[username],one=True)["count"]
    return render_template(
        "/myprofile.html", username=username, email=email, is_logged=is_logged,unseen_number=unseen_number
    )
